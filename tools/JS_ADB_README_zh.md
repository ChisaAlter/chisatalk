...
在 ChisaTalk 仓库里，它们是主要的 **安卓端 JavaScript 运行、调试与验证入口**。
...
- 把一个 JS 文件临时推到设备
- 在 ChisaTalk 的 Android JS runtime 里调用某个导出函数
- 返回结构化结果
...
```cmd
set CHISATALK_RESULT_WAIT_SECONDS=30
...
它就会把这个文件放到设备上，在 ChisaTalk 的 JS runtime 里直接调用这个函数。
