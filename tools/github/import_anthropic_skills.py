...
def _http_json(
    ...
    headers: dict[str, str] = {
        ...
        "User-Agent": "ChisaTalkTools/1.0 (import_anthropic_skills.py)",
    }
    if token:
...
def _http_text(
    ...
    for attempt in range(retries):
        ...
        req.add_header("User-Agent", "ChisaTalkTools/1.0 (import_anthropic_skills.py)")
...
def _ai_generate_description(
    ...
    user_prompt = textwrap.dedent(
        f"""
        Write a short Chinese description for an ChisaTalk Skill Market listing.
...
def _build_operit_issue_body(description: str, repository_url: str, version: str) -> str:
    ...
    lines.append("## 📦 安装方式")
    lines.append("")
    lines.append("1. 打开 ChisaTalk → 包管理 → Skills")
    ...
    lines.append("| 项目 | 值 |")
    lines.append("|------|-----|")
    lines.append("| 发布平台 | ChisaTalk Skill 市场 |")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Import skills from awesome-agent-skills into ChisaTalk Skill Market by creating GitHub issues."
    )
    parser.add_argument(
        "--env",
    ...
    parser.add_argument(
        "--target-repo",
        default="ChisaTalk/ChisaTalkSkillMarket",
