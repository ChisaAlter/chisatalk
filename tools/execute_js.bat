...
if "%~1"=="" (
    ...
    echo Note: set CHISATALK_RESULT_WAIT_SECONDS to customize result wait, default is 15 seconds
    exit /b 1
)
...
set "CHISATALK_JSON_FILE=%LOCAL_PARAMS_FILE%"
set "CHISATALK_JSON_CONTENT=%PARAMS_ARG%"
set "CHISATALK_JSON_LEGACY_MODE=%PARAMS_NEEDS_LEGACY_NORMALIZATION%"
powershell -NoProfile -Command "$raw=$env:CHISATALK_JSON_CONTENT; if ($env:CHISATALK_JSON_LEGACY_MODE -eq 'true') { $normalized=$raw -replace '^\{\\\s+','{\"' -replace '\\,\\','\",\"' -replace ',\\',',\"' -replace '\\:','\":' -replace ':\\',':\"' -replace '\\}','\"}' -replace '\\/','/' } else { $normalized=$raw }; [System.IO.File]::WriteAllText($env:CHISATALK_JSON_FILE, $normalized, [System.Text.UTF8Encoding]::new($false))"
set "CHISATALK_JSON_FILE="
set "CHISATALK_JSON_CONTENT="
set "CHISATALK_JSON_LEGACY_MODE="
if errorlevel 1 (
