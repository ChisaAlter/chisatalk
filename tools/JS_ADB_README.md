...
This repo's ADB JS executors are the main **device-side JavaScript runtime launchers** for ChisaTalk.
...
- Run JS directly inside the ChisaTalk Android runtime
...
- `tools/execute_js.bat` / `tools/execute_js.sh`
  ...
  Pushes one JS file to the Android device and executes an exported function inside the ChisaTalk runtime.
...
```bash
CHISATALK_RESULT_WAIT_SECONDS=30 ./execute_js.sh path/to/your/script.js functionName '{"param1":"value1"}'
```
...
It pushes that file to the device and calls that function inside ChisaTalk's Android JS runtime.
