...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "device external packages directory, and trigger ChisaTalk's debug install flow."
        )
    )
    parser.add_argument(
        "source",
    ...
    parser.add_argument(
        "--log-wait-seconds",
        type=int,
        default=int(os.environ.get("CHISATALK_LOG_WAIT_SECONDS", DEFAULT_LOG_WAIT_SECONDS)),
        help=(
            "Seconds to wait after sending the install broadcast before dumping logcat. "
            "Defaults to CHISATALK_LOG_WAIT_SECONDS or 6."
        ),
    )
