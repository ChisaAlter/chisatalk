...
REM - set CHISATALK_RESULT_WAIT_SECONDS to customize result wait, default is 15 seconds
...
if "%CHISATALK_RESULT_WAIT_SECONDS%"=="" (
    set "RESULT_WAIT_SECONDS=15"
) else (
    set "RESULT_WAIT_SECONDS=%CHISATALK_RESULT_WAIT_SECONDS%"
)
...
set "BUNDLED_FILE="
set "CHISATALK_TEMP_EXT=.js"
set "CHISATALK_TEMP_PREFIX=operit_js_bundle_"
for /f "usebackq delims=" %%T in (`powershell -NoProfile -Command "$tmp = [System.IO.Path]::GetTempFileName(); $ext = $env:CHISATALK_TEMP_EXT; $prefix = $env:CHISATALK_TEMP_PREFIX; $dir = [System.IO.Path]::GetDirectoryName($tmp); $base = [System.IO.Path]::GetFileNameWithoutExtension($tmp); $target = Join-Path $dir ($prefix + $base + $ext); Move-Item -LiteralPath $tmp -Destination $target -Force; Write-Output $target"`) do (
    set "BUNDLED_FILE=%%T"
)
set "CHISATALK_TEMP_EXT="
set "CHISATALK_TEMP_PREFIX="
...
if defined LOCAL_PARAMS_FILE (
    ...
    set "CHISATALK_TEMP_EXT=.json"
    set "CHISATALK_TEMP_PREFIX=operit_js_params_"
    for /f "usebackq delims=" %%T in (`powershell -NoProfile -Command "$tmp = [System.IO.Path]::GetTempFileName(); $ext = $env:CHISATALK_TEMP_EXT; $prefix = $env:CHISATALK_TEMP_PREFIX; $dir = [System.IO.Path]::GetDirectoryName($tmp); $base = [System.IO.Path]::GetFileNameWithoutExtension($tmp); $target = Join-Path $dir ($prefix + $base + $ext); Move-Item -LiteralPath $tmp -Destination $target -Force; Write-Output $target"`) do (
        set "LOCAL_PARAMS_FILE=%%T"
    )
    set "CHISATALK_TEMP_EXT="
    set "CHISATALK_TEMP_PREFIX="
    call set "CHISATALK_JSON_FILE=%%LOCAL_PARAMS_FILE%%"
    if not defined CHISATALK_JSON_FILE (
        echo Error: Failed to allocate local params temp file
        exit /b 1
    )
    set "CHISATALK_JSON_CONTENT=%PARAMS_ARG%"
    powershell -NoProfile -Command "[System.IO.File]::WriteAllText($env:CHISATALK_JSON_FILE, $env:CHISATALK_JSON_CONTENT, [System.Text.UTF8Encoding]::new($false))"
    set "CHISATALK_JSON_FILE="
    set "CHISATALK_JSON_CONTENT="
    if errorlevel 1 (
...
set "CHISATALK_TEMP_EXT=.json"
set "CHISATALK_TEMP_PREFIX=operit_js_result_"
for /f "usebackq delims=" %%T in (`powershell -NoProfile -Command "$tmp = [System.IO.Path]::GetTempFileName(); $ext = $env:CHISATALK_TEMP_EXT; $prefix = $env:CHISATALK_TEMP_PREFIX; $dir = [System.IO.Path]::GetDirectoryName($tmp); $base = [System.IO.Path]::GetFileNameWithoutExtension($tmp); $target = Join-Path $dir ($prefix + $base + $ext); Move-Item -LiteralPath $tmp -Destination $target -Force; Write-Output $target"`) do (
...
set "CHISATALK_RESULT_FILE=%LOCAL_RESULT_FILE%"
powershell -NoProfile -Command ^
  "$ErrorActionPreference = 'Stop';" ^
  "$path = $env:CHISATALK_RESULT_FILE;" ^
  ...
  "if ($env:CHISATALK_SHOW_RAW_RESULT -eq '1') { Write-Host ''; Write-Host 'Raw JSON :'; Write-Host $raw; }"
if errorlevel 1 (
