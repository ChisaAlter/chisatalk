...
class GitHubForgePublishService(
    ...
    private suspend fun ensureForgeRepository(
        ...
        if (!failureMessage.contains("HTTP 404")) {
            return Result.failure(existingRepo.exceptionOrNull() ?: Exception("Failed to load ChisaTalkForge"))
        }
        ...
        return githubApiService.createRepository(
            ...
            description = "ChisaTalk publish-only artifact repository for release assets.",
            isPrivate = false,
            autoInit = true
        ).map { repo ->
            ForgeRepoInfo(
    ...
    private suspend fun initializeForgeRepository(
        ...
        return githubApiService.createTextFile(
            ...
            repo = repo,
            path = "README.md",
            message = "Initialize ChisaTalkForge repository",
            textContent =
                buildString {
                    appendLine("# ChisaTalkForge")
                    appendLine()
                    appendLine("This repository stores release assets published from ChisaTalk.")
                }
        )
    }
