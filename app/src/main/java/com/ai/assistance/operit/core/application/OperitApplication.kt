...
/** Application class for ChisaTalk */
class OperitApplication : Application(), ImageLoaderFactory, WorkConfiguration.Provider {
    ...
    override fun onCreate() {
        ...
        AppLogger.d(TAG, "ChisaTalk 普通模式已关闭 WorkflowScheduler 启动初始化")
        ...
        AppLogger.d(TAG, "ChisaTalk 普通模式已关闭无障碍服务预绑定")
            ...
            try {
                val deletedFiles =
                    cleanDirectory(File(OperitPaths.cleanOnExitPathSdcard()), preserveRootNoMedia = true) +
                        cleanDirectory(File(cacheDir, "ChisaTalk/cleanOnExit"), preserveRootNoMedia = false)
                AppLogger.d(
                    TAG,
