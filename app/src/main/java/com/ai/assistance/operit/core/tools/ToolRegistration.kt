 ...
 */
fun registerChisaTalkBasicTools(handler: AIToolHandler, context: Context) {
            ...
            descriptionGenerator = { tool ->
                ...
                when {
                    !visitKey.isNullOrBlank() && !linkNumber.isNullOrBlank() ->
                            s(R.string.toolreg_visit_web_search_link_desc, linkNumber, visitKey.take(8))
                    !url.isNullOrBlank() -> s(R.string.toolreg_visit_web_url_desc, url)
                    else -> s(R.string.toolreg_visit_web_desc)
                }
            },
            executor = { tool ->
                ToolGetter.getWebVisitTool(context).invoke(tool)
            }
    )
    ...
    handler.registerTool(
            ...
            descriptionGenerator = { tool ->
                ...
                s(R.string.toolreg_read_file_desc, path, formatEnvInfo(environment))
            },
    ...
    handler.registerTool(
            ...
            descriptionGenerator = { tool ->
                ...
                s(R.string.toolreg_read_file_part_desc, rangeInfo, path, formatEnvInfo(environment))
            },
            executor = { tool -> runBlocking(Dispatchers.IO) { fileSystemTools.readFilePart(tool) } }
    )
    ...
    handler.registerTool(
            ...
            descriptionGenerator = { tool ->
                val path = tool.parameters.find { it.name == "path" }?.value ?: ""
                val environment = tool.parameters.find { it.name == "environment" }?.value
                s(R.string.toolreg_read_file_full_desc, path, formatEnvInfo(environment))
            },
