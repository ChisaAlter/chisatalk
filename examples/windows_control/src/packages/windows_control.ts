/* METADATA
{
    ...
    "description": {
        "zh": "通过 HTTP 调用 ChisaTalk PC Agent 控制 Windows 电脑，支持执行 PowerShell/CMD 命令并返回输出。",
        "en": "Control a Windows PC through ChisaTalk PC Agent over HTTP, execute PowerShell/CMD commands, and return output."
    },
    ...
    "env": [
        {
            ...
            "description": { "zh": "ChisaTalk PC Agent 地址，例如 http://192.168.1.8:58321", "en": "ChisaTalk PC Agent URL, e.g. http://192.168.1.8:58321" },
            "required": true
        },
        {
    ...
    "tools": [
        ...
        },
        {
            "name": "windows_exec",
            "description": {
                "zh": "通过 ChisaTalk PC Agent 的 HTTP 接口在 Windows 上执行命令。",
                "en": "Execute commands on Windows via ChisaTalk PC Agent HTTP API."
            },
            "parameters": [
                {
                    "name": "command",
...
const windowsControl = (function () {
    ...
    async function persistWindowsExecOutputIfTooLong(
        ...
        const filePath = `${CHISATALK_CLEAN_ON_EXIT_DIR}/windows_exec_output_${timestamp}_${rand}.log`;
        ...
        return {
            ...
            outputChars,
            operitCleanOnExitDir: CHISATALK_CLEAN_ON_EXIT_DIR,
    ...
    async function windows_test_connection(params?: WindowsTestConnectionParams): Promise<WindowsExecResult> {
        try {
            ...
            const timeoutMs = Math.min(
                parseTimeout(params?.timeout_ms, CONNECTION_TEST_TIMEOUT_MS),
