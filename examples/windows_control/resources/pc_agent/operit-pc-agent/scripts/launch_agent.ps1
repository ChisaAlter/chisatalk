...
function Start-AgentAndProbeReady {
    ...
    $previousBindOverride = $env:CHISATALK_BIND_ADDRESS_OVERRIDE
    try {
        if ([string]::IsNullOrWhiteSpace($BindAddressOverride)) {
            Remove-Item Env:CHISATALK_BIND_ADDRESS_OVERRIDE -ErrorAction SilentlyContinue
        }
        else {
            $env:CHISATALK_BIND_ADDRESS_OVERRIDE = $BindAddressOverride
        }
        ...
        if ($null -eq $previousBindOverride) {
            ...
            $env:CHISATALK_BIND_ADDRESS_OVERRIDE = $previousBindOverride
        }
    }
...
$mutexName = "Local\ChisaTalkPcAgentLauncher"
...
try {
    ...
    Write-Host "[OK] ChisaTalk PC Agent started on $url"
