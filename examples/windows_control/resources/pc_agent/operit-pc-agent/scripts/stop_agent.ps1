...
try {
    ...
    if (-not $hasLock) {
        ...
        Write-Host "[WARN] ChisaTalk PC Agent is busy (launcher running)."
        exit 1
    }
    ...
    if ($stopped) {
        Write-Host "[OK] ChisaTalk PC Agent stopped (port $port)."
        Write-Log "OK" "Agent stopped on port $port"
    }
    else {
        Write-Host "[OK] No running ChisaTalk PC Agent process found."
