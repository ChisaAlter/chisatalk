...
function parseChisaTalkEntryArray(rawEntries: unknown[]): ParsedImportPayload | null {
  ...
  for (const rawEntry of rawEntries) {
    ...
    const name = requireStringField(rawEntry, "name", "ChisaTalk 条目 name");
    const content = requireStringField(rawEntry, "content", "ChisaTalk 条目 content");
    if (!name || !content) {
      continue;
    }
    ...
    entries.push({
      name,
      content,
      keywords: readOptionalKeywordArrayField(rawEntry, "keywords", "ChisaTalk 条目 keywords"),
      is_regex: readOptionalBooleanField(rawEntry, "is_regex", false, "ChisaTalk 条目 is_regex"),
      case_sensitive: readOptionalBooleanField(rawEntry, "case_sensitive", false, "ChisaTalk 条目 case_sensitive"),
      always_active: readOptionalBooleanField(rawEntry, "always_active", false, "ChisaTalk 条目 always_active"),
      enabled: readOptionalBooleanField(rawEntry, "enabled", true, "ChisaTalk 条目 enabled"),
      priority: readOptionalNumberField(rawEntry, "priority", 50, "ChisaTalk 条目 priority"),
      scan_depth: readOptionalNumberField(rawEntry, "scan_depth", 0, "ChisaTalk 条目 scan_depth"),
      ...
      insertion_depth: readOptionalNumberField(rawEntry, "insertion_depth", 0, "ChisaTalk 条目 insertion_depth")
    });
  }
...
function parseImportedWorldBookPayload(raw: unknown): ParsedImportPayload {
  if (Array.isArray(raw)) {
    const parsedChisaTalk = parseChisaTalkEntryArray(raw);
    if (parsedChisaTalk) {
      return parsedChisaTalk;
    }
  }
  ...
  throw worldBookError(
    ...
    "暂不支持该世界书格式，当前支持 ChisaTalk、SillyTavern 独立 lorebook，以及角色卡内嵌 character_book"
  );
}
