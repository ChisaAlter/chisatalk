...
function getToolOutputText(result: unknown): string {
    ...
    const captureFromOutput = extractMarkedBlock(output, "__CHISATALK_TMUX_CAPTURE_BEGIN__", "__CHISATALK_TMUX_CAPTURE_END__");
    if (captureFromOutput) {
        return captureFromOutput;
    }
    const captureFromRaw = extractMarkedBlock(rawOutput, "__CHISATALK_TMUX_CAPTURE_BEGIN__", "__CHISATALK_TMUX_CAPTURE_END__");
    if (captureFromRaw) {
