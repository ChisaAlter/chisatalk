...
const linuxSshTools = (function () {
    ...
    async function persistResultOutputIfTooLong(data, fileLabel) {
        ...
        await Tools.Files.mkdir(CHISATALK_CLEAN_ON_EXIT_DIR, true);

        const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
        const rand = Math.floor(Math.random() * 1_000_000);
        const safeLabel = firstNonBlank(fileLabel, "linux_ssh_output")
            .replace(/[^a-zA-Z0-9._-]+/g, "_");
        const filePath = `${CHISATALK_CLEAN_ON_EXIT_DIR}/${safeLabel}_${timestamp}_${rand}.log`;
        ...
        return {
            ...
            operit_clean_on_exit_dir: CHISATALK_CLEAN_ON_EXIT_DIR,
    ...
    async function ensureRemoteTmuxWindow(config, requestedWindowName) {
        const script = [
            ...
            "fi",
            "printf '__CHISATALK_TMUX_WINDOW_READY__\\n'",
        ...
        const success = result.exitCode === 0 && !result.timedOut && result.output.includes("__CHISATALK_TMUX_WINDOW_READY__");
    ...
    async function readRemoteFileContent(config, path, lineStart, lineEnd, useSudo) {
        ...
        const script = [
            ...
            "  echo '__CHISATALK_FILE_NOT_FOUND__'",
            "  exit 4",
            "fi",
            "printf '__CHISATALK_BEGIN__\\n'",
            readCmd,
            "printf '\\n__CHISATALK_END__\\n'"
        ].join("\n");
        ...
        const content = extractBlock(result.output, "__CHISATALK_BEGIN__", "__CHISATALK_END__");
        return {
            output: result.output,
            content
        };
    ...
    async function linux_ssh_test_connection(params) {
        return await runTool(async () => {
            ...
            const block = extractBlock(result.output, "__CHISATALK_CONNECT_BEGIN__", "__CHISATALK_CONNECT_END__");
    ...
    async function linux_ssh_tmux_run(params) {
        return await runTool(async () => {
            ...
            const script = [
                ...
                "printf '__CHISATALK_TMUX_RUN_OK__\\n'",
            ...
            const success = result.exitCode === 0 && !result.timedOut && result.output.includes("__CHISATALK_TMUX_RUN_OK__");
    ...
    async function linux_ssh_tmux_capture(params) {
        return await runTool(async () => {
            ...
            const script = [
                `tmux has-session -t ${shellQuote(tmuxSessionName)} 2>/dev/null || { echo '__CHISATALK_TMUX_NOT_FOUND__'; exit 4; }`,
                "printf '__CHISATALK_TMUX_CAPTURE_BEGIN__\\n'",
                `tmux capture-pane -t ${shellQuote(target)} -p -S -${maxLines}`,
                "printf '\\n__CHISATALK_TMUX_CAPTURE_END__\\n'"
            ].join("\n");
            ...
            const success = result.exitCode === 0 && !result.timedOut;
            const content = extractBlock(result.output, "__CHISATALK_TMUX_CAPTURE_BEGIN__", "__CHISATALK_TMUX_CAPTURE_END__");
    ...
    async function linux_ssh_tmux_list_windows(params) {
        return await runTool(async () => {
            ...
            const script = [
                `tmux has-session -t ${shellQuote(tmuxSessionName)} 2>/dev/null || { echo '__CHISATALK_TMUX_NOT_FOUND__'; exit 4; }`,
                "printf '__CHISATALK_TMUX_WINDOWS_BEGIN__\\n'",
                `tmux list-windows -t ${shellQuote(tmuxSessionName)} -F '#{window_index}:#{window_name}'`,
                "printf '__CHISATALK_TMUX_WINDOWS_END__\\n'"
            ].join("; ");
            ...
            const notFound = hasExactMarkerLine(result.output, "__CHISATALK_TMUX_NOT_FOUND__");
            const success = result.exitCode === 0 && !result.timedOut && !notFound;
            const block = extractBlock(result.output, "__CHISATALK_TMUX_WINDOWS_BEGIN__", "__CHISATALK_TMUX_WINDOWS_END__");
            const windows = block
                .split(/\r?\n/)
    ...
    async function linux_ssh_tmux_input(params) {
        return await runTool(async () => {
            ...
            scriptLines.push("printf '__CHISATALK_TMUX_INPUT_OK__\\n'");
            ...
            const success = result.exitCode === 0 && !result.timedOut && result.output.includes("__CHISATALK_TMUX_INPUT_OK__");
    ...
    async function linux_ssh_tmux_close(params) {
        return await runTool(async () => {
            ...
            const script = [
                `tmux has-session -t ${shellQuote(tmuxSessionName)} 2>/dev/null || { echo '__CHISATALK_TMUX_NOT_FOUND__'; exit 4; }`,
                `tmux list-windows -t ${shellQuote(tmuxSessionName)} -F '#{window_name}' | grep -Fx -- ${shellQuote(windowName)} >/dev/null || { echo '__CHISATALK_TMUX_WINDOW_NOT_FOUND__'; exit 5; }`,
                `tmux kill-window -t ${shellQuote(targetWindow)}`,
                "printf '__CHISATALK_TMUX_CLOSE_OK__\\n'"
            ].join("\n");
            ...
            const sessionExists = !hasExactMarkerLine(result.output, "__CHISATALK_TMUX_NOT_FOUND__");
            const windowExists = !hasExactMarkerLine(result.output, "__CHISATALK_TMUX_WINDOW_NOT_FOUND__");
            const success = result.exitCode === 0 && !result.timedOut && sessionExists && windowExists && result.output.includes("__CHISATALK_TMUX_CLOSE_OK__");
