/* METADATA
{
  ...
  display_name: {
    zh: "ChisaTalk平台编辑器"
    en: "ChisaTalk Platform Editor"
  }
  description: {
    zh: '''ChisaTalk 平台配置直改工具包：提供一组可直接读取与修改 ChisaTalk 平台设置的工具，覆盖 MCP、Skill、Sandbox Package、功能模型绑定、模型参数、上下文总结与 TTS/STT 语音服务配置。'''
    en: '''Direct ChisaTalk platform configuration toolkit: a collection of tools for reading and directly modifying ChisaTalk platform settings, covering MCP, Skill, Sandbox Package, function-model bindings, model parameters, context-summary settings, and TTS/STT speech-service configuration.'''
  }
...
【Skill：安装与排查】
1) 目录：/sdcard/Download/ChisaTalk/skills/
2) 识别规则：每个 Skill 必须是一个文件夹，且包含 SKILL.md（skill.md 也可）。
3) 添加方式（按这个做）：
- 先从可信来源下载 Skill（zip 或仓库源码均可）
- 把下载内容解压后，直接放到 /sdcard/Download/ChisaTalk/skills/
- 最终目录结构必须是 /sdcard/Download/ChisaTalk/skills/<skill_name>/，且该目录内有 SKILL.md
4) 元数据解析：
...
5) 制作包文档：
- https://cdn.jsdelivr.net/gh/ChisaTalk/ChisaTalk@main/docs/SCRIPT_DEV_SKILL.md
...
1) Config directory: /sdcard/Download/ChisaTalk/mcp_plugins/
...
1) Directory: /sdcard/Download/ChisaTalk/skills/
...
- Extract it, then place the folder directly under /sdcard/Download/ChisaTalk/skills/
- Final structure must be /sdcard/Download/ChisaTalk/skills/<skill_name>/ and that folder must contain SKILL.md
4) Metadata parsing:
...
1. 先创建目录：/sdcard/Download/ChisaTalk/skills/<skill_name>/
2. 必备文件：SKILL.md
...
1. Create a directory: /sdcard/Download/ChisaTalk/skills/<skill_name>/
...
6. Practical tip: download an existing skill, extract it directly, and ensure the directory contains SKILL.md`;
    ...
    const TOOLPKG_DEBUG_INSTALL_COMPONENT = "com.chisatalk.app/com.ai.assistance.operit.core.tools.packTool.ToolPkgDebugInstallReceiver";
    const SANDBOX_SCRIPT_EXECUTION_ACTION = "com.chisatalk.app.EXECUTE_JS";
    ...
    async function resolve_toolpkg_source(rawSourcePath) {
        ...
        if (sourceType === "directory") {
            ...
            const tempExtractDir = path_join(CHISATALK_CLEAN_ON_EXIT_DIR, `operit_editor_toolpkg_extract_${safe_debug_file_stem(lowerBaseName.replace(/\.toolpkg$/i, ""), "toolpkg")}_${Date.now()}`);
    ...
    async function build_toolpkg_archive_from_folder(source) {
        const tempBuildDir = path_join(CHISATALK_CLEAN_ON_EXIT_DIR, `operit_editor_toolpkg_build_${safe_debug_file_stem(source.packageId, "toolpkg")}_${Date.now()}`);
