...
export const REMOTE_CHISATALK_SETUP_ZH_CN: RemoteChisaTalkSetupI18n = {
  title: "远程 ChisaTalk 配置",
  subtitle: "让当前 ChisaTalk 通过局域网 HTTP API 调用另一台设备上的 ChisaTalk。",
  topBanner:
    "先在目标设备的 ChisaTalk 设置里启用“外部 HTTP 调用”，再把该设备页面里显示的局域网地址和 Bearer Token 填到这里。",
  configCardTitle: "连接配置",
  configCardSubtitle:
    "保存后，当前 ChisaTalk 就能把聊天请求转发到另一台设备上的 ChisaTalk。推荐填写完整的 http://IP:端口。",
  baseUrlLabel: "远程 ChisaTalk Base URL",
  ...
  errorBaseUrlRequired: "请填写远程 ChisaTalk 的 Base URL",
  errorTokenRequired: "请填写远程 ChisaTalk 的 Bearer Token",
  ...
  exampleCardSubtitle:
    "导入并启用后，当前 ChisaTalk 可以直接把任务转发到另一台设备。常用入口是 remote_operit_chat。",
  examplePromptTitle: "推荐场景",
  examplePromptBody:
    "当另一台设备拥有本机没有的文件、应用、权限或上下文时，让当前 ChisaTalk 调 remote_operit_chat 去协作。",
