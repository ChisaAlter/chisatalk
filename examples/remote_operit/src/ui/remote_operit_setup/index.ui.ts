...
import { resolveRemoteChisaTalkSetupI18n, type RemoteChisaTalkSetupI18n } from "../../i18n";

const REMOTE_CHISATALK_PACKAGE_NAME = "remote_operit";
const REMOTE_CHISATALK_TOOL_CONFIGURE = "remote_operit_configure";
const REMOTE_CHISATALK_TOOL_TEST_CONNECTION = "remote_operit_test_connection";

const KEY_BASE_URL = "REMOTE_CHISATALK_BASE_URL";
const KEY_TOKEN = "REMOTE_CHISATALK_TOKEN";
const KEY_TIMEOUT_MS = "REMOTE_CHISATALK_TIMEOUT_MS";
...
function resolveText(): RemoteChisaTalkSetupI18n {
  ...
  return resolveRemoteChisaTalkSetupI18n(preferredLocale);
}
...
async function ensureImportedAndUsed(ctx: ComposeDslContext, packageName: string, text: RemoteChisaTalkSetupI18n): Promise<void> {
...
export default function Screen(ctx: ComposeDslContext): ComposeNode {
  ...
  const callRemoteTool = async (toolName: string, params?: Record<string, unknown>): Promise<Record<string, unknown>> => {
    const packageName = resolveRuntimePackageName(ctx, REMOTE_CHISATALK_PACKAGE_NAME);
    ...
    const candidates = [
      ...
      `${REMOTE_CHISATALK_PACKAGE_NAME}:${toolName}`
  ...
  const checkConnectionByTool = async (): Promise<void> => {
    ...
    const result = await callRemoteTool(REMOTE_CHISATALK_TOOL_TEST_CONNECTION, {
  ...
  const saveConfig = async (shouldTest: boolean): Promise<void> => {
    ...
    try {
      const result = await callRemoteTool(REMOTE_CHISATALK_TOOL_CONFIGURE, {
        base_url: baseUrl,
        token,
