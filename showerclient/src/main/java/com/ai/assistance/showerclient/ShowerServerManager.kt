...
object ShowerServerManager {
    ...
    suspend fun ensureServerStarted(context: Context): Boolean {
        ...
        // 3) Copy the jar from /sdcard/Download/ChisaTalk to /data/local/tmp using shell identity,
    ...
    private suspend fun copyJarToExternalDir(context: Context): File = withContext(Dispatchers.IO) {
        // Reuse the same base directory as screenshots: /sdcard/Download/ChisaTalk
        val baseDir = File("/sdcard/Download/ChisaTalk")
        if (!baseDir.exists()) {
            baseDir.mkdirs()
        }
