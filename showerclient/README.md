...
```kotlin
class ChisaTalkShowerShellRunner : ShellRunner {
...
class YourApplication : Application() {
    override fun onCreate() {
        ...
        ShowerEnvironment.shellRunner = ChisaTalkShowerShellRunner()
    }
}
```
...
- 运行时库会自动从自身 `assets` 中读取，并复制到 `/sdcard/Download/ChisaTalk/shower-server.jar`，再拷贝到 `/data/local/tmp/shower-server.jar`。
