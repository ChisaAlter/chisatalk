...
**ToolPkg** 是 ChisaTalk 项目中用于打包和分发工具包的标准格式。它允许开发者将多个相关的工具脚本、资源文件和 UI 模块打包成一个单一的、易于分发和管理的文件。
  ...
  "subpackages": [
    {
      ...
      "description": {
        "zh": "通过 ChisaTalk PC Agent 控制 Windows",
        "en": "Control Windows via ChisaTalk PC Agent"
      }
    }
  ],
  "resources": [
...
/* METADATA
{
    ...
    "description": {
        "zh": "通过 HTTP 调用 ChisaTalk PC Agent 控制 Windows 电脑",
        "en": "Control a Windows PC through ChisaTalk PC Agent over HTTP"
    },
    ...
    "env": [
        {
            ...
            "description": {
                "zh": "ChisaTalk PC Agent 地址",
                "en": "ChisaTalk PC Agent URL"
            },
            "required": true
        }
    ],
...
用户可以通过以下方式导入外部包：
1. 将 `.toolpkg` 文件复制到设备的 `Android/data/com.chisatalk.app/files/packages/` 目录
2. 在应用中使用"导入包"功能
...
4. 通过 `adb push` 将包推送到手机的 `Android/data/com.chisatalk.app/files/packages/`
...
- `--log-wait-seconds <n>`：发送广播后等待多少秒再抓取日志；默认读取 `CHISATALK_LOG_WAIT_SECONDS`，否则为 `6`
...
- 这个脚本依赖手机上的 ChisaTalk 已包含 `ToolPkgDebugInstallReceiver` 调试广播入口；如果手机装的是旧版本 App，广播不会生效。
