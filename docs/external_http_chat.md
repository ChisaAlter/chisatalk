...
本文档描述 ChisaTalk 新增的局域网 HTTP 聊天接口。它与现有 `EXTERNAL_CHAT` Intent 接口语义一致，只是入口从广播改成了 HTTP。
...
AI 完成后，ChisaTalk 会向 `callback_url` 发送一次 `POST application/json` 回调，请求体仍然是：
