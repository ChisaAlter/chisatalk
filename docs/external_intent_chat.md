...
本文档描述一个**独立于工作流系统**的外部交互接口：外部应用通过发送广播 Intent（`com.chisatalk.app.EXTERNAL_CHAT`）向 ChisaTalk 发起一次“发送消息给 AI”的请求，并通过另一个广播接收执行结果。
...
- **请求 Action**：`com.chisatalk.app.EXTERNAL_CHAT`
- **默认回传 Action**：`com.chisatalk.app.EXTERNAL_CHAT_RESULT`
...
| `reply_action` | `String` | 否 | `com.chisatalk.app.EXTERNAL_CHAT_RESULT` | 指定回传广播 action |
...
ChisaTalk 在处理完成后会发送一条广播（action 为 `reply_action` 或默认 action），携带如下 extras：
...
adb shell am broadcast \
  -a com.chisatalk.app.EXTERNAL_CHAT \
...
ChisaTalk 会在处理完成后发送广播回传：

- 默认 action：`com.chisatalk.app.EXTERNAL_CHAT_RESULT`
...
这样 ChisaTalk 在回传时会对广播设置 `intent.setPackage(reply_package)`。
...
class ExternalChatResultReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action != "com.chisatalk.app.EXTERNAL_CHAT_RESULT") return
...
<receiver
    ...
    <intent-filter>
        <action android:name="com.chisatalk.app.EXTERNAL_CHAT_RESULT" />
    </intent-filter>
</receiver>
```
