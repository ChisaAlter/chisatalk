...
本文档说明如何通过 **Intent 广播**触发 ChisaTalk 的工作流（Workflow）。
...
为了确保广播能稳定投递给 ChisaTalk，推荐使用：

- **显式广播**：指定 `component`（包名 + Receiver 类名）

这样即使 action 是自定义的，也能确保发给 ChisaTalk 的 `WorkflowTaskerReceiver`。

---

## 2. Receiver / Component 信息

- **Receiver 类**：`com.chisatalk.app.integrations.tasker.WorkflowTaskerReceiver`
- **包名**：`com.chisatalk.app`
- **Component**：`com.chisatalk.app/.integrations.tasker.WorkflowTaskerReceiver`
...
- **action**：填写你希望外部触发的 action，例如：
  - `com.example.myapp.TRIGGER_CHISATALK_WORKFLOW_A`
...
adb shell am broadcast \
  -n com.chisatalk.app/.integrations.tasker.WorkflowTaskerReceiver \
  -a com.example.myapp.TRIGGER_CHISATALK_WORKFLOW_A \
  --es message "hello from adb" \
  --es request_id "req-1001"
```

- `-n` 指定 component，确保发给 ChisaTalk。
...
但这取决于系统版本、ROM 策略以及 ChisaTalk 是否能在后台接收该隐式广播。

### 4.3 方式 C：使用内置默认 action（兼容用法）

如果你的工作流 Trigger 里 `action` 配置的是默认值：

- `com.chisatalk.app.TRIGGER_WORKFLOW`

那么可以使用：

```bash
adb shell am broadcast \
  -a com.chisatalk.app.TRIGGER_WORKFLOW \
  --es message "hello" \
...
- **action**：`com.chisatalk.app.WORKFLOW_RESULT`
- **extra_key**：`result`
...
- Action 填：`com.chisatalk.app.WORKFLOW_RESULT`
- 在 Task 中读取变量（通常可直接用 `%result` 或从 extras 映射中取）

### 6.2 写一个最小接收 App / Receiver（用于调试）

在你的测试 App 中注册一个 `BroadcastReceiver` 监听 `com.chisatalk.app.WORKFLOW_RESULT`，在 `onReceive()` 里读取：
